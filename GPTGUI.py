# -*- coding: utf-8 -*-
"""
Created on Thu Dec 19 19:10:35 2024
...
class WorkerThread(QThread):
    update_image = pyqtSignal(np.ndarray)  # Signal to send images to the main thread
    update_status = pyqtSignal(str)       # Signal to update status messages
    ...
    def run(self):
        for command in self.series:
            ...
            if len(command) == 1:
                delay = command[0]
                self.update_status.emit(f"Delaying for {delay} sec")
                time.sleep(delay)
            else:
                ...
                for _ in range(num_exposures):
                    ...
                    self.update_image.emit(image)  # Emit the image to the GUI
                    self.update_status.emit(f"Captured image for {file_name}")
                    time.sleep(exposure_time)
        self.update_status.emit("Series complete")
        ...
        for row in rows:
            ...
            if parts[0] == 'add':
                if len(parts) != 3 or parts[1] != 'delay':
                    self.updateCameraStatus("ERROR: Incorrect delay syntax")
                    return
                try:
                    delay_duration = float(parts[2])
                except ValueError:
                    self.updateCameraStatus("ERROR: Delay duration is not a number")
                    return
                total_time += delay_duration
                series.append([delay_duration])
            else:
                try:
                    ...
                    self.updateCameraStatus("ERROR: Invalid exposure syntax")
                    return
                series.append([num_exposures, exposure_time, file_name])

        self.start_series(series)

    def start_series(self, series):
        self.worker_thread = WorkerThread(series)
        self.worker_thread.update_image.connect(self.update_plot)
        self.worker_thread.update_status.connect(self.updateCameraStatus)
